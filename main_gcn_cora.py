...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a simple GCN on Cora dataset.")
    # 常用超参数示例，实际使用中可按需调整
    ...
    # 数据集路径配置（对于 Planetoid 数据集无需指定路径，直接加载即可）
    parser.add_argument("--dataset", type=str, default="Cora", choices=["Cora", "Citeseer", "PubMed"], help="选择使用的数据集")

    # 模型参数
    parser.add_argument("--dropout_prob", type=float, default=0.5, help="Dropout 概率")

    # 其它可选参数
    parser.add_argument("--log_dir", type=str, default="./logs/cora_gcn/", help="日志文件夹路径")
    ...
    parser.add_argument("--save_path", type=str, default="./checkpoints/cora_gcn/best_model.pth",
...
def main():
    ...
    # 初始化 Trainer
    trainer = TrainerGCN(model, criterion, optimizer, device, scheduler=scheduler)
    ...
    for epoch in range(args.epochs):
        ...
        # ---------- Training ---------- #
        train_loss, train_acc = trainer.train_one_epoch(data, train_mask)

        # 获取当前学习率
        current_lr = trainer.get_current_lr()

        # ---------- Validation ---------- #
        val_loss, val_acc = trainer.validate_one_epoch(data, val_mask)
    ...
    test_loss, test_acc = trainer.test_model(data, test_mask)
