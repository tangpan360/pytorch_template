...
from models import SAGEPoolNet

from torch_geometric.loader import DataLoader

from dataset_class.yoochoose_dataset import YooChooseBinaryDataset

# 导入封装好的类和函数
from utils import TrainerSAGEPoolNet
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a SAGE on YooChoose dataset.")
    # 常用超参数示例，实际使用中可按需调整
    ...
    # 数据集路径配置
    parser.add_argument("--dataset_name", type=str, default="yoochoose", help="图数据集名称")

    # 模型参数
    parser.add_argument("--dropout_prob", type=float, default=0.5, help="Dropout 概率")

    # 其它可选参数
    parser.add_argument("--log_dir", type=str, default="./logs/yoochoose_sage/", help="日志文件夹路径")
    ...
    parser.add_argument("--save_path", type=str, default="./checkpoints/yoochoose_sage/yoochoose_best_model.pth",
...
def main():
    ...
    # 加载 YooChoose 数据集
    dataset = YooChooseBinaryDataset(root='./data/yoochoose/')
    ...
    # 初始化 Trainer
    trainer = TrainerSAGEPoolNet(model, criterion, optimizer, device, scheduler=scheduler)
